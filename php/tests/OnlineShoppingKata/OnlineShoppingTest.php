...
class OnlineShoppingTest extends TestCase
{
    ...
    public function testWarehouseUseCaseWithItems()
    {
        $cart = new Cart();
        $cart->addItem($this->cherryBloom); 
        $cart->addItem($this->rosePetal);
    ...
    //test that event is in the new store even if it 
    //wasnt there originally
    public function testHomeToPickup()
    {
        $cart = new Cart();
        ...
        $hasItem = $this->backaplan->hasItem($this->makeoverNordstan);
